package com.tangykiwi.kiwiclient.modules.render.seedray;
        ...
        if (worldSeed != null) {
            ...
            for (int range = 0; range <= rangeVal; range++) {
                for (int x = -range + chunkX; x <= range + chunkX; x++) {
                    renderChunk(x, chunkZ + range - rangeVal, event.getMatrixStack());
                }
                for (int x = (-range) + 1 + chunkX; x < range + chunkX; x++) {
                    renderChunk(x, chunkZ - range + rangeVal + 1, event.getMatrixStack());
                }
            }
        }
    }
